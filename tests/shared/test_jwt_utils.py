...
def test_verify_jwt_rejects_invalid_signature() -> None:
    ...
    token = create_mailcheck_jwt("a" * MIN_JWT_SECRET_LENGTH, issued_at)

    with pytest.raises(RuntimeError, match="JWT signature invalid"):
        verify_mailcheck_jwt(token, "b" * MIN_JWT_SECRET_LENGTH, max_age_seconds=60)
...
def test_verify_jwt_rejects_invalid_subject() -> None:
    ...
    with pytest.raises(RuntimeError, match="JWT subject invalid"):
        verify_mailcheck_jwt(token, SECRET, max_age_seconds=60)
...
def test_verify_jwt_rejects_invalid_issuer() -> None:
    ...
    with pytest.raises(RuntimeError, match="JWT invalid"):
        verify_mailcheck_jwt(token, SECRET, max_age_seconds=60)
...
def test_verify_jwt_rejects_short_secret() -> None:
    ...
    with pytest.raises(RuntimeError, match="must be at least"):
        verify_mailcheck_jwt(token, "short", max_age_seconds=60)
